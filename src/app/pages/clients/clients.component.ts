...
const NAME_SIZE_MIN = 2
const SURNAME_SIZE_MIN = 2
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ButtonComponent, InputComponent, ReactiveFormsModule, MatFormFieldModule, MatInputModule, PlusComponent, TableComponent,  MatDialogModule,
    MatButtonModule, MatDividerModule, ModalRemoveComponent
  ],
...
export class ClientsComponent implements OnInit {
  ...
  myForm : FormGroup = new FormGroup({
    "name": new FormControl("", [Validators.required, Validators.minLength(NAME_SIZE_MIN)]),
    "surname": new FormControl("", [Validators.required, Validators.minLength(SURNAME_SIZE_MIN)]),
    "email": new FormControl("", [Validators.required, Validators.email]),
    "phone": new FormControl("", [Validators.required, phoneValidator()]),
  });
  
  clients$!: BehaviorSubject<Client[]>
  private _chosenClientEmail!: string

  @ViewChild(TableComponent, {static: false})
  private table: TableComponent | undefined

  constructor(private clientStore: ClientsStoreService, public dialog: MatDialog){}
  ...
  protected addClient() {
    const newClient = this.myForm.value as Client
    this.clientStore.addClient(newClient)
  }

  protected editCLient() {
    const newClient = this.myForm.value as Client
    this.clientStore.editClient(this._chosenClientEmail, newClient)
  }

  protected deleteClients() {
    const selectedClients = this.table?.selection.selected
    if (selectedClients) {
      this.clientStore.deleteClients(selectedClients)
    }
  ...
  private setValueToControl(client: Client) {
    this.myForm.setValue(client)
    this._chosenClientEmail = client.email
  }

  protected openModalAddClient(enterAnimationDuration: string, exitAnimationDuration: string){
    ...
    dialogRef.afterClosed().pipe(first()).subscribe(addClient => {
      if (addClient === true) {
        this.addClient()
      }
      this.resetForm()
  ...
  protected openModalEditClient(client: Client, enterAnimationDuration: string, exitAnimationDuration: string){
    this.setValueToControl(client)
    const dialogRef = this.dialog.open(ModalAddComponent, {
      width: '448px',
      enterAnimationDuration,
      exitAnimationDuration,
    ...
    dialogRef.afterClosed().pipe(first()).subscribe(isEditedClient => {
      if (isEditedClient === true) {
        this.editCLient()
      }
      this.resetForm()
    })
  ...
  protected openModalDeleteClients(enterAnimationDuration: string, exitAnimationDuration: string) {
    const selectedClientsCount = this.table?.selection.selected.length
    ...
    dialogRef.afterClosed().pipe(first()).subscribe(isRemoveClients => {
      if (isRemoveClients === true) {
        this.deleteClients()
      }
    })
  }
